...
export function App(){
  ...
  const [user,setUser]=useState(null); const [variant,setVariant]=useState('A'); const [showUpgrade,setShowUpgrade]=useState(false); const [ats,setAts]=useState([]);
  ...
  async function fetchMe(){ try{ const r=await fetch(API_BASE+'/api/auth/me',{credentials:'include'}); const j=await r.json(); if(j.ok){ setUser(j.user); fetchQuota(); } }catch(_){ } }
  async function fetchQuota(){ try{ const r=await fetch(API_BASE+'/api/usage/summary',{credentials:'include'}); const j=await r.json(); if(j.ok) setQuota(j); }catch(_){ } }
  async function upgrade(){ if(!user){ if(window.confirm('Devi accedere per effettuare upgrade. Torna alla home?')) window.location.href='/'; return; } fetch(API_BASE+'/api/ab/event',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event:'upgrade_click',variant})}); const r=await fetch(API_BASE+'/api/pro/upgrade',{method:'POST',credentials:'include'}); const j=await r.json(); if(j.ok){ alert('Sei Pro!'); setUser({...user,is_pro:1}); setShowUpgrade(false); fetchQuota(); } }
  function handleUse(key){ fetch(API_BASE+'/api/usage',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({toolKey:key})}); if(key==='quote'){ fetchQuota(); } window.open(`/tool.html?tool=${key}`,'_blank','noopener'); }
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:30}}>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
        {!user && <button className="btn" onClick={()=>window.location.href='/'}>Login / Registrati</button>}
        {user && <span style={{fontSize:12}}> {(user.name||user.email)} {user.is_pro? '(Pro)':''}</span>}
      </div>
    </div>
    {quotaInfo}
