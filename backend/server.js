...
app.post('/api/export/quote', async (req,res)=>{
  ...
  const primary='#1e3a8a'; const light='#64748b'; const currency=payload.currency||'EUR'; function money(n){ try{ return new Intl.NumberFormat('it-IT',{style:'currency',currency}).format(Number(n||0)); }catch{ return (currency==='EUR'?'€':'')+Number(n||0).toFixed(2);} }
