function FlashcardTool(){
  ...
  function importJson(){
    try { const parsed = JSON.parse(syncJson); if(Array.isArray(parsed)) setCards(parsed); else setMsg('JSON non valido (array richiesto)'); }
    catch(err){ setMsg('Errore parse JSON: '+err.message); }
  }
  function exportJSON(){
  ...
  async function exportPDF(){
    ...
    try{
      const r = await fetch('/api/export/flashcards',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cards})});
      if(!r.ok){ const j = await r.json(); setMsg('Errore: '+j.error); return; }
      const blob = await r.blob();
      const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='flashcards.pdf'; a.click(); URL.revokeObjectURL(url); setMsg('PDF pronto');
    }catch(err){ setMsg('Errore rete: '+err.message); }
  }
  return (
    <div className="card">
      <h3>Flashcard generator</h3>
      <p style={{fontSize:12,opacity:.7,margin:'4px 0 10px'}}>Crea rapidamente flashcard domanda/risposta. Puoi esportare in JSON oppure PDF per stampa.
      Suggerimento: mantieni le domande concise e una sola risposta diretta.</p>
