...
function App(){
  ...
  useEffect(()=>{
    ...
    // Poll ATS suggestions
    const id = setInterval(()=>{
      fetch('/api/ats').then(r=>r.json()).then(data=>{
  ...
  async function register(){
    ...
    if(!email) return;
    const r = await fetch('/api/auth/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email})});
  ...
  async function upgrade(){
    ...
    fetch('/api/ab/event',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event:'upgrade_click',variant})});
    const r = await fetch('/api/pro/upgrade',{method:'POST',headers:{'x-session-token':token}});
    const j = await r.json(); if(j.ok){ alert('Sei Pro!'); setUser({...user,is_pro:1}); setShowUpgrade(false); }
  }

  function handleUse(key){
    fetch('/api/usage', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ toolKey:key }) });
    // open in new tab dedicated page
    window.open(`/tool.html?tool=${key}`,'_blank','noopener');
  }

  useEffect(()=>{
    if(showUpgrade){
      fetch('/api/ab/event',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event:'banner_view',variant})});
    }
  },[showUpgrade,variant]);
  ...
  return (
    <div>
      ...
      {/* Panel embedded non più necessario quando apriamo in nuova scheda; si può mantenere come anteprima futura se serve */}
